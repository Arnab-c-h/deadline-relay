...
- The local app completed real cross-provider execution and independent read-back on September 14. Live approval guardrails and actual backend interruption/recovery passed, including a fix for resume-status polling. See `LIVE-ACCEPTANCE.md`. These local results do not count as deployed acceptance.
- Hosted database, durable queue, owner authentication, hosted OAuth, deployed packaging, and hosted acceptance remain implementation work.

Server-owned credentials for the dedicated provider destinations are configured locally. Hosted database and Vercel destination credentials will be requested when their setup becomes necessary. Keep secrets in local environment files or the deployment's server-side environment configuration.
